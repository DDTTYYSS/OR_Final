...
class InterviewSchedulerOptimized:
    ...
    def calculate_applicant_priority(self, applicant):
        """Calculate priority for applicant - prioritize most time-constrained applicants."""
        # Calculate total available time hours for this applicant
        total_available_minutes = 0
        min_available_for_any_dept = float('inf')
        dept_availability = {}
        ...
        for dept in self.departments:
            ...
            dept_availability[dept] = dept_minutes
            total_available_minutes += dept_minutes
            
            if dept_minutes > 0:
                min_available_for_any_dept = min(min_available_for_any_dept, dept_minutes)
        
        if min_available_for_any_dept == float('inf'):
            min_available_for_any_dept = 0
        
        # Convert minutes to hours for easier understanding
        total_hours = total_available_minutes / 60
        min_hours = min_available_for_any_dept / 60
        
        # Calculate urgency score - lower total time = higher urgency = higher priority
        # We want to prioritize applicants with less available time
        urgency_score = (total_available_minutes * 1000 + min_available_for_any_dept * 100)
        
        # Additional penalty for applicants with very limited options
        if total_hours < 2:  # Less than 2 hours total
            urgency_score -= 100000  # Very high priority
        elif total_hours < 4:  # Less than 4 hours total  
            urgency_score -= 50000   # High priority
        
        # Check for "critical" applicants (very few time slots)
        critical_depts = sum(1 for minutes in dept_availability.values() if 0 < minutes < 60)  # Less than 1 hour
        if critical_depts > 0:
            urgency_score -= critical_depts * 20000
